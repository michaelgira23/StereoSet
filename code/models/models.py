...
class ModelNSP(nn.Module):
    def __init__(self, pretrained_model, device, nsp_dim=300):
        super(ModelNSP, self).__init__()
        if pretrained_model != 'antibias':
            self.pretrained2model = {
                "xlnet": "XLNetModel", "bert": "BertModel", "roberta": "RobertaModel", "gpt2": "GPT2Model"}
            self.model_class = self.pretrained2model[pretrained_model.lower().split(
                "-")[0]]
            self.core_model = getattr(
                transformers, self.model_class).from_pretrained(pretrained_model)
        else:
            self.core_model = antibias_model(device)
        ...
        hidden_size = self.core_model.config.hidden_size if pretrained_model != 'antibias' else 768
