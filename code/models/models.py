...
from .unprejudiced.model import model as unprejudicedModel
...
class BertNextSentence(transformers.BertPreTrainedModel):
    ...
    def __new__(self, pretrained_model):
        return transformers.BertForNextSentencePrediction.from_pretrained(
            pretrained_model)
...
class RoBERTaLM(transformers.BertPreTrainedModel):
    ...
    def __new__(self, pretrained_model):
        return transformers.RobertaForMaskedLM.from_pretrained(
            pretrained_model)
...
class XLMLM(transformers.BertPreTrainedModel):
    ...
    def __new__(self, pretrained_model):
        return transformers.XLMWithLMHeadModel.from_pretrained(
            pretrained_model)
...
class GPT2LM(transformers.GPT2PreTrainedModel):
    ...
    def __new__(self, pretrained_model):
        if pretrained_model == 'gpt2-unprejudiced':
            return unprejudicedModel
...
class ModelNSP(nn.Module):
    def __init__(self, pretrained_model, nsp_dim=300):
        ...
        self.model_class = self.pretrained2model[
            pretrained_model.lower().split("-")[0]]
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, next_sentence_label=None, \
        ...
        if 'Roberta' in self.model_class or 'GPT2' in self.model_class:
            ...
            outputs = self.core_model(input_ids,
                                      token_type_ids=token_type_ids,
